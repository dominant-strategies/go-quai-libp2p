...
func AddGenesisUtxos(utxoView *types.UtxoViewpoint, nodeLocation common.Location, logger *log.Logger) {
	...
	for addressString, utxo := range qiAlloc {
		...
		utxo := &types.UtxoEntry{
			...
			PackedFlags:  types.TfModified,
		}

		utxoView.AddEntry(outpoint, utxo)
	}
}
