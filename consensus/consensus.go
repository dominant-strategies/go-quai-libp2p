...
type Engine interface {
	...
	DeltaLogS(header *types.WorkObject) *big.Int
	...
	Finalize(chain ChainHeaderReader, header *types.WorkObject, state *state.StateDB, txs []*types.Transaction,
		uncles []*types.WorkObject)
	...
	// Note: The block header and state database might be updated to reflect any
	// consensus rules that happen at finalization (e.g. block rewards).
	FinalizeAndAssemble(chain ChainHeaderReader, woHeader *types.WorkObjectHeader, state *state.StateDB, txs []*types.Transaction, uncles []*types.WorkObject, etxs []*types.Transaction, manifest types.BlockManifest, receipts []*types.Receipt) (*types.WorkObject, error)
	...
	Seal(header *types.WorkObjectHeader, results chan<- *types.WorkObjectHeader, stop <-chan struct{}) error

	// CalcDifficulty is the difficulty adjustment algorithm. It returns the difficulty
	// that a new block should have.
	CalcDifficulty(chain ChainHeaderReader, parent *types.WorkObject) *big.Int
	...
	VerifySeal(header *types.WorkObject) (common.Hash, error)
