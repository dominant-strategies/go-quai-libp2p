...
func CalculateRewardForQi(header *types.WorkObject) map[uint8]uint8 {
...
func CalculateRewardForQiWithFees(header *types.WorkObject, fees *big.Int) map[uint8]uint8 {
	rewardFromDifficulty := new(big.Int).Add(types.Denominations[types.MaxDenomination], types.Denominations[10])
	reward := new(big.Int).Add(rewardFromDifficulty, fees)
	return findMinDenominations(reward)
}

func CalculateRewardForQiWithFeesBigInt(header *types.WorkObject, fees *big.Int) *big.Int {
	rewardFromDifficulty := new(big.Int).Add(types.Denominations[types.MaxDenomination], types.Denominations[10])
	reward := new(big.Int).Add(rewardFromDifficulty, fees)
	return reward
}
